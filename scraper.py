...
def scrape_batch(COUNTRY,TRACKIN_FUNC,tracking_nums_batch,scraping_url,output_path,country_logger,log_country_dir_path,c_audit,dfs,scraping_tracking_nos):
    ...
    for i in tracking_nums_batch:
        threads.append(twrv.ThreadWithReturnValue(target=TRACKIN_FUNC, args=(i,scraping_tracking_nos,scraping_url,country_logger,log_country_dir_path,)))
...
def scrape_list(COUNTRY,TRACKIN_FUNC,tracking_nums,batch_size,scraping_url,output_path,logger,log_dir_path,c_audit,output_dir_path):
    ...
    for i in chunks(tracking_nums,batch_size):
        scrape_batch(COUNTRY,TRACKIN_FUNC,i,scraping_url,output_path,country_logger,log_country_dir_path,c_audit,dfs,scraping_tracking_nos)
